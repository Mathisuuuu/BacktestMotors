...
MODE_D_EMPLOI = [
    ...
    "qu'elle attendait.",
    "",
    "NE PAS SE REPETER. Un bloc `definitions` a la racine nomme une grandeur une",
    "seule fois ; partout ailleurs, `{\"$ref\": \"nom\"}` la rappelle. C'est la",
    "convention de JSON Schema, et elle s'applique a toute profondeur, y compris",
    "dans `risk.sizing.signal`. Une definition peut en referencer une autre.",
    "Mesure du 2026-09-16 : la plus grosse specification du depot passe de 984 a",
    "546 lignes, et son `config_hash` ne bouge PAS - la substitution est",
    "textuelle, faite avant toute validation, et c'est la forme DEVELOPPEE qui",
    "est hachee. Ecrire une grandeur deux fois n'est donc pas une erreur, mais",
    "rien ne verifiera jamais que les deux copies disent la meme chose.",
    "",
    "    {",
    "      \"definitions\": {",
    "        \"sigma\": {\"type\": \"rolling\", \"stat\": \"stdev\", \"window\": 60,",
    "                   \"inner\": {\"type\": \"price\", \"field\": \"close\"}},",
    "        \"bande_haute\": {\"type\": \"arith\", \"op\": \"+\",",
    "                        \"left\": {\"type\": \"primitive\", \"ref\": \"sma@1\",",
    "                                 \"params\": {\"window\": 60}},",
    "                        \"right\": {\"$ref\": \"sigma\"}}",
    "      },",
    "      \"strategy\": {\"ref\": \"rules@1\", \"params\": {\"rules\": {",
    "        \"entry_long\": {\"type\": \"compare\", \"op\": \">\",",
    "                       \"left\": {\"type\": \"price\", \"field\": \"close\"},",
    "                       \"right\": {\"$ref\": \"bande_haute\"}}",
    "      }}}",
    "    }",
    "",
    "Une definition vaut n'importe quelle valeur JSON, pas seulement un noeud :",
    "un seuil ecrit quatre fois gagne autant a etre nomme qu'un arbre de trente",
    "lignes.",
]
...
CONTRAINTES = [
    ...
    "",
    "`definitions` et `$ref` sont, comme `note`, des mots RESERVES : le premier",
    "a la racine du document, le second comme CLE a toute profondeur. Cinq",
    "ecritures sont refusees, chacune parce qu'elle produirait un resultat",
    "lisible mais faux : un nom inconnu, un cycle entre definitions, un `$ref`",
    "accompagne d'autres cles (elles seraient perdues en silence), un pointeur",
    "de la forme '#/definitions/x' au lieu du nom seul, et une definition",
    "JAMAIS referencee - ce dernier cas etant la copie devenue orpheline que le",
    "bloc sert justement a empecher.",
    "",
