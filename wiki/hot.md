...
**Activite par type :** note × 91, fix × 46, feat × 45, decision × 20, mesure × 17, essai × 4, experiment × 2, setup × 1, lint × 1, audit × 1, refactor × 1, perf × 1, bug × 1, correction × 1
...
- **2026-09-15** — feat | **Les quatre exemples INTRADAY entrent aux empreintes : 7 -> 11** | les sept archivees etaient toutes QUOTIDIENNES ou mensuelles, donc `session`, `cumulative`, `session_lag`, `rolling.across`, `event`, `exogenous` et `session_only` n'etaient proteges par RIEN. Ce n'etait pas theorique : le defaut `is_last`, qui marquait 71,9 % des barres, a survecu precisement pour cette raison - « invisible aux 7 empreintes parce qu'elles sont toutes quotidiennes ». Le trou est ferme
- **2026-09-15** — feat | **L'audit des pieges couvre desormais l'AUTRE moitie du vocabulaire** : les 32 champs de `risk.sizing`, `execution` et `data[]` | `test_pieges.py` ne rendait l'audit obligatoire que pour les 27 types de NOEUDS. Or le piege qui a fait ecarter `vol_target` de la replication Zarattini vit dans `risk.sizing` : **`vol_window` compte des BARRES**, donc sur du minute il mesure une volatilite par minute - un facteur `sqrt(390)`, pres de vingt. Les modeles pydantic n'ont pas de decorateur ou declarer un piege : ils passent par `json_schema_extra` de leur `Field`, ce qui garde la propriete qui compte - le piege vit A COTE de la definition et bouge avec elle. Trois pieges declares, chacun tracant une erreur DATEE : `vol_window` (barres), `data[].session_only` (548 seances de dimanche sans lui) et `sizing.contracts` (BASE tronquee sous `vol_target` : `int(1 x 0.9) = 0`, la strategie ne prend jamais position sans lever). Effet de bord VOULU : les pieges entrent dans le JSON Schema publie - un piege connu du seul code source ne previent personne. 17 tests, garde-fou verifie en simulant un champ neuf
- **2026-09-15** — mesure | **La surface de Sharpe culmine exactement a 1,5, le choix du papier - mais a 1,14 et non 1,472** | cinq multiplicateurs mesures a troncature neutralisee : 1,00 -> 0,88 (1 774 trades) · 1,25 -> 1,11 (1 333) · **1,50 -> 1,14 (1 001)** · 2,05 -> 0,84 (598) · 2,50 -> 0,45 (301). Le taux de reussite monte de facon MONOTONE avec la largeur de bande - 34,33 % a 44,52 % - pendant que le nombre de trades s'effondre : aucune largeur ne reproduit a la fois les 957 trades ET les 42,8 % du papier. **Consequence** : l'hypothese « 1,472 est le maximum d'un balayage tombe ailleurs » est ECARTEE - il tombe au MEME endroit. Et reproduire l'argmax est une information EN FAVEUR de notre implementation : une specification fautive aurait deplace l'optimum. Le residu de 0,33 ne se rattache plus a aucune cause testable par un run : restent une difference de DONNEES, ou un 1,472 non reproductible
